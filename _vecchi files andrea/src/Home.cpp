...
std::vector<CycleDevice*> CycleDeviceList = {new CycleDevice("Lavatrice",-2,110,false),new CycleDevice("Lavastoviglie",-1.5,195,false),new CycleDevice("Tapparelle elettriche",-0.3,1,false),new CycleDevice("Microonde",-0.8,2,false),new CycleDevice("Asciugatrice",-0.5,60,false),new CycleDevice("Televisore", -0.2 ,60,false)};
...
float Home::checkMax(float cmgQt,short timee){
    float actPower = 0;
    for (Device* d : ManualDeviceList){
        if(d->getStateAt(timee)){
            actPower += d->getPowerCons();
        }
    }
    for (CycleDevice* c : CycleDeviceList){
        if(c->getStateAt(timee)){
            actPower += c->getPowerCons();
        }
    }
    std::cout<<"<"<<actPower<<"|"<<cmgQt<<"|"<<(actPower+cmgQt)<<">";
    return actPower+cmgQt;
}
...
void Home::goForward(short ti)
{
    for (Device* d : ManualDeviceList)
    {
        d->refreshTimers(ti,*this);
    }
    for (CycleDevice* c : CycleDeviceList)
    {
        c->refreshTimers(ti,*this);
    }
    this->time = ti;
    return;
}
